...
- Classify embedding prices by the commercial input being metered: token rates are `input_text`, processed images are `input_image`, and explicit audio/video duration rates are `input_audio`/`input_video`. Use the generic `embedding` meter only for an embedding operation price whose input modality is not already expressed by a directional meter. Product/model wording alone is not a modality condition, and a modality encoded by the meter is not duplicated as applicability.
