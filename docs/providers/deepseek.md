...
All documents are public, exact URLs. Deterministic HTML/schema extraction verifies the model enum, usage fields, cache semantics, account deductions, route mappings, and future-price notice used by normalization. Missing documents, changed commercial claims, an unknown price row, a partially parsed model column, or an API-reference disagreement rejects the provider refresh. Third-party price books are never fallbacks.
...
- The current price page announces a future peak/off-peak policy: peak hours will cost 2× regular rates for every billing item during 09:00–12:00 and 14:00–18:00 Beijing time. It gives no effective date and says activation depends on a future announcement. Keep the six current regular facts active and record the future multiplier as unbound; the daily contract will fail when the wording changes so the policy can be normalized only once effective applicability is known.
...
- ccusage obtains these prices through LiteLLM and therefore adds no independent DeepSeek billing authority. None of the three sources represents the announced not-yet-effective peak schedule, the Anthropic wildcard routing rule, account grants, or a request-level charged-cost API.
