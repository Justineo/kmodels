...
- Optional fixed first-party companions come from the current `platform-docs-public` source tree and cover prompt caching, Batch, regional inference, the
  ...
  create model identities. Inside an acquired bundle, one drifting claim withholds only its dependent fact; when a configured companion cannot be acquired at all, provider-atomic publication retains the previous complete pricing partition instead of mixing fresh and retained commercial evidence.
...
- Explicit free prices remain exact when the provider publishes units. A callable
  ...
  `not_published`, not free. Retired definitions have no current hosted offer, so
  historical repository amounts are excluded. Their state is `not_applicable` unless
  an independently published weight distribution remains attached; that external
  commercial path keeps the model-level price unresolved while its distribution book
  remains `externally_billed`.
