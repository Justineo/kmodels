...
- Normalize explicit token, character, image, frame, second, and request prices. A fixed-duration song is one request, not a per-second rate.
- Detect damaged labeled-price cells from repeated table structure rather than a model-specific exception. For the same model and semantic column across regional tables, accept a unique label sequence observed at least twice; if an outlier is that complete sequence plus only trailing repeated labels, canonicalize the supported prefix and retain the rejected suffix as informational raw evidence. Any other anomaly remains a commercial raw fact.
- When one price cell contains unqualified, explicit 5m, and explicit 1h Claude cache-write labels, collapse the unqualified label only if its amount exactly duplicates the 5m amount. The explicit TTL-qualified rates remain canonical; unequal or structurally incomplete groups remain raw.
- Parse page/token equivalence only from the structured model-card `Model ID` and quota row, using the quantities published for input and output rather than a model allowlist. Collapse OCR token/page alternatives only when exact decimal arithmetic proves both page prices from those quantities. Preserve all other per-page alternatives as raw facts because their billing applicability is unknown.
- Prefer a billing unit identified by the official structured Cloud SKU table when a pricing cell also publishes a secondary display unit. Retain the full published cell as informational evidence. Without matching model, meter, and SKU-unit evidence, preserve simultaneous unequal units and let canonical assembly report the ambiguity.
