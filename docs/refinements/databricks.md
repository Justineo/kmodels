...
The 49 rows exactly match the reviewed AWS regional serving catalog. Their IDs come from labeled `Endpoint name` fields, which are the Databricks request identities. The lifecycle result contains 45 active, three deprecated, and one retired row; five rows separately carry preview maturity. The official task matrix classifies 46 endpoints as general-purpose text generation and three as embeddings; two general-purpose rows additionally produce images.

All 49 rows have exact model-specific `/serving-endpoints/{name}/invocations` route evidence. Forty-five rows have published DBU pricing and four remain unknown. The 472 live rates retain endpoint, context tier, batch, cache-read, provisioned entry/scaling capacity, promotion, and effective date. DBU is not converted to USD. The workspace inventory remains optional, so a documented endpoint is not proof that a particular workspace or region exposes it.
...
Embedding operations are outside Kong's documented Databricks capability. Image-output rows may participate only through their separately evidenced General purpose/chat operation; the image-generation operation itself is not a Kong compatibility claim. Deployment compatibility still requires active lifecycle, an acceptable release stage, and workspace/region availability.

## Refinement decision

1. Keep the exact endpoint-name identities and DBU-native prices.
2. Use the exact General purpose task set, not name heuristics, when projecting active chat endpoints with an acceptable release stage to Kong.
