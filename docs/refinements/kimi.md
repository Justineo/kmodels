...
Nineteen rows are reasonable as the union of current OpenAPI model discriminators, current and retired model tables, pricing tables, and exact changelog matches. The snapshot contains four active, seven legacy, seven retired, and one unknown-state row. The legacy rows remain usable by existing users but are closed to newly registered users, so they are not mislabeled as deprecated. All are text-generation models. The twelve IDs in the OpenAPI discriminator receive the exact `/v1/chat/completions` route; rows observed only in historical or pricing sources do not inherit it.
