...
The 100 rows are plausible for the union of regional Google, partner, and managed open-model catalogs. The lifecycle snapshot contains 64 active, 17 deprecated, and 19 retired rows; 18 rows separately carry preview maturity. Eight rows expose multiple normalized operations; 58 rows have published prices and 42 remain unknown.
...
Partner and open-model rows do not become compatible merely because they are `text_generation`. Compatibility requires the publisher/API family and the exact Vertex method used by that model. Region remains part of the relation.
