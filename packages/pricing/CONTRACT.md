...
Components are billing aggregation instances, not logs to deduplicate. The caller owns request, attempt, item, job, resource, and session correlation. A single unambiguous applicable binding boundary may be inferred. Otherwise supply `aggregation`. If an offer has multiple boundaries, represent them as explicitly linked components with the appropriate boundaries. Two components are linked when either lists the other in `relatedComponents`; links are symmetric for relations, aggregation hand-off, and duplicate charging. A charge belonging to another boundary is resolved there only when such a linked component prices the same rate term at the same unit price; otherwise it remains unresolved. Never sum durations across independent instances before applying an increment or minimum.
...
Never choose the lowest or first of different potentially applicable rates. Equal semantic variants may resolve without a selector only when their union covers the remaining selected offer state; when equivalent states themselves resolved by coverage, that state scope is the union of all of them, never one representative. When the selected state is not `numeric`, or the published states exclude the supplied selectors, no published rate produces a charge. Coverage is checked over the finite cells induced by the published categorical values and numeric boundaries; above 4,096 cells the selector remains unresolved. Differing simultaneously selected variants are conflicts. Equivalent evidence variants do not add another charge.
...
A contribution references existing rate terms and evaluates them using the contribution's binding. It does not copy the price or imply another request. Raw facts on the referenced rate remain unresolved for the contribution. Duplicate rate/binding charges within a component are invalid. Charging a contribution and its referenced rate within one component, or in components linked as the same event, is also invalid. Independent, unlinked billing instances may legitimately use the same rate.
...
Only allowances with canonical reset `none` are evaluated within the component and its explicit outgoing links. Coverage can zero resolved target charges. A quantity allowance reduces one compatible target quantity with a zero floor before repricing at the unit price of the variant that was charged. A credit reduces one matching-denomination amount with a zero floor. Target evidence is preserved on adjusted charges. Overlapping allowances, any unresolved requirement on a target component, allocation across multiple charges for a quantity/credit benefit, rate substitution, or longer reset periods remain explicit gaps. In particular, an organization/month allowance cannot be consumed from one request without account state. Known subtotals in these cases retain unallocated gross amounts; they are not final invoice totals.
...
Malformed data/input is a thrown `PricingError` with stable `code`; message text is diagnostic and not a portable assertion. Data and requests must be I-JSON values. `ARITHMETIC_LIMIT` is raised only by the exact-integer digit bound; an internal evaluator failure is not relabelled with a contract code:
