...
const endpointReferences: Readonly<
  Record<string, readonly (readonly [string, readonly RegExp[]])[]>
> = {
...
function pricingDocument(body: string): LoadedDocument {
  const source = load(body);
  if (source(".devsite-article-body").length > 0) return source;
  const normalized = load('<main><div class="devsite-article-body"></div></main>');
  const article = normalized(".devsite-article-body");
  source("h1,h2,h3,h4,table").each((_index, element) => {
    const item = source(element);
    if (element.tagName !== "table") {
      const label = text(item.text());
      if (label !== "" && !/Consumption model ID:/i.test(label)) article.append(item.clone());
      return;
    }
    ...
    headers.each((index, cell) => {
      if (/Flexible Savings Plan/i.test(text(source(cell).text()))) omitted.add(index);
    });
    if (omitted.size > 0)
      table.find("tr").each((_rowIndex, row) => {
        const cells = source(row).find("th,td");
        const offset = Math.max(0, headers.length - cells.length);
        cells.each((cellIndex, cell) => {
          if (omitted.has(cellIndex + offset)) source(cell).remove();
        });
      });
...
function reviewedEndpointReferences(
  sourceId: string,
  documents: LinkedDocument[],
): ReadonlySet<string> {
  const reviewed = new Set<string>();
  for (const [path, patterns] of endpointReferences[sourceId] ?? []) {
    ...
    if (document !== undefined && patterns.every((pattern) => pattern.test(value)))
      reviewed.add(path);
  }
  return reviewed;
}
...
function modelEndpoints(
  ...
  if (sourceId === "vertex-google-models") {
    if (/\/models\/gemini\//.test(path)) {
      ...
      return model.tasks.includes("speech_to_speech") ||
        !references.has("/gemini-enterprise-agent-platform/models/start")
        ? undefined
        : [endpoints.generate];
    }
    if (
      /\/models\/veo\//.test(path) &&
      references.has("/gemini-enterprise-agent-platform/models/video/generate-videos-from-text")
    )
      return [endpoints.video];
    if (
      /\/models\/lyria\//.test(path) &&
      references.has("/gemini-enterprise-agent-platform/models/music/generate-music")
    )
      return [endpoints.predict];
    if (
      /\/models\/imagen\//.test(path) &&
      references.has("/vertex-ai/generative-ai/docs/image/generate-images")
    )
      return [endpoints.predict];
  }
  if (sourceId === "vertex-partner-models") {
    if (
      /\/partner-models\/claude\//.test(path) &&
      references.has("/gemini-enterprise-agent-platform/models/partner-models/claude/use-claude")
    )
      return [endpoints.claude, endpoints.claudeStream];
    if (
      /\/partner-models\/grok\//.test(path) &&
      references.has("/gemini-enterprise-agent-platform/models/partner-models/grok/responses")
    )
      return [endpoints.grok];
    if (
      /\/partner-models\/llama\//.test(path) &&
      references.has("/gemini-enterprise-agent-platform/models/partner-models/llama/use-llama")
    )
      return [endpoints.llama];
  }
  return undefined;
}
...
function parseModelTables(
  ...
  path: string,
  body: string,
  endpointEvidence: ReadonlySet<string>,
): boolean {
  const $ = load(body);
  ...
  tables.each((_index, tableElement) => {
    ...
    model.api_endpoints = modelEndpoints(input.source.id, path, $, model, endpointEvidence);
...
function pricingEvidence(documents: LinkedDocument[]): PricingEvidence {
...
function tiers(
  ...
  if (/\bFlex\s*\/\s*Batch\b/i.test(header)) {
    if (/\(Batch\)/i.test(scope)) return ["batch"];
    if (/\(Flex\)/i.test(scope)) return ["flex"];
    return ["flex", "batch"];
  }
...
function column(cells: Selection, columnCount: number, index: number): Selection | undefined {
  if (index < 0) return undefined;
  const cellIndex = index - Math.max(0, columnCount - cells.length);
  return cellIndex < 0 || cellIndex >= cells.length ? undefined : cells.eq(cellIndex);
}
...
function pageAlternativeVerified(
  ...
  equivalence: PageTokenEquivalence | undefined,
): boolean {
  const match = raw.match(
    /\$(\d+(?:\.\d+)?)\s*(?:\/\s*(?:1M|million)\s*tokens?)?\s*\(\s*or\s*\$(\d+(?:\.\d+)?)\s*\/\s*page\s*\)/i,
  );
  if (match?.[1] === undefined || match[2] === undefined || equivalence === undefined) return false;
  const tokensPerPage = meter.startsWith("input_")
    ? equivalence.inputTokensPerPage
    : meter.startsWith("output_")
      ? equivalence.outputTokensPerPage
      : undefined;
  return (
    tokensPerPage !== undefined &&
    decimalsEqual(scaleDecimal(multiplyDecimal(match[1], String(tokensPerPage)), -6), match[2])
  );
}
...
function excludedPricingTable(table: Selection, headers: string[]): boolean {
  const section = text(table.prevAll("h2,h3,h4").first().text());
  return (
    /^(?:Agents|CodeMender|Model Tuning|AlphaEvolve|Agent Platform Model Optimizer|Provisioned Throughput|Context Cache Storage)/i.test(
      section,
    ) ||
    headers.some((header) => /training|storage|GSU/i.test(header)) ||
    /\b(?:training|storage)\b/i.test(text(table.text()))
  );
}
        ...
        cells.each((cellIndex, cell) => {
          ...
          if (alternative >= 0)
            for (const target of current)
              for (const rateMeter of rateMeters)
                if (
                  !pageAlternativeVerified(
                    raw,
                    rateMeter,
                    evidence.pageTokenEquivalences.get(target.model_id),
                  )
                )
                  rawRate(
                    target,
                    sourceId,
                    `${modelLabel || target.name}; ${header}`,
                    raw,
                    {
                      deployment_scope: deploymentScope(scope),
                      region:
                        scope === "" || deploymentScope(scope) !== undefined ? undefined : scope,
                      ...contextTokenBounds(header),
                      ...validity,
...
function labeledTables(
  ...
  $(".devsite-article-body table").each((_index, tableElement) => {
    ...
    if (
      ...
      headers.length > 3 ||
      excludedPricingTable(table, headers) ||
      !/(?:Input|Output|Cache (?:Write|Hit)):\s*\$/i.test(text(table.text()))
    )
      return;
  ...
  $(".devsite-article-body table").each((_tableIndex, tableElement) => {
    ...
    table
      ...
      .each((_rowIndex, row) => {
        ...
        const prices = [...rowText.matchAll(/\$(\d+(?:\.\d+)?)/g)]
          .flatMap((candidate) => (candidate[1] === undefined ? [] : [candidate[1]]))
          .filter((candidate) => !decimalsEqual(candidate, "0"));
        const price = match?.[1] ?? prices.at(-1);
        if (price === undefined) {
          reconcile?.({
            disposition: "explicit_non_numeric",
...
function claudeWebSearchPricing(
  ...
  const rows = $(".devsite-article-body table")
    .filter((_index, table) => {
      const headers = tableHeaders($, $(table));
      return headers[0] === "Tool" && /^(?:Price|Price \(USD\))$/.test(headers[1] ?? "");
    })
    .find("tr")
    .slice(1)
...
export function parseVertexCatalog(input: Input): ProviderModel[] {
  ...
  for (const document of bundle.documents) {
    ...
    const hasModelCard = parseModelTables(models, input, path, document.body, endpointEvidence);
  ...
  if (
    input.source.id === "vertex-open-models" &&
    endpointEvidence.has("/gemini-enterprise-agent-platform/models/maas/call-open-model-apis")
  )
    applyOpenExamples(models, bundle.documents);
  ...
  if (
    embedding !== undefined &&
    endpointEvidence.has("/gemini-enterprise-agent-platform/models/embeddings/get-text-embeddings")
  )
    applyEmbeddingReference(models, embedding.body);
...
export function parseVertexPricing(input: Input): ProviderModel[] {
  ...
  for (const target of input.catalogModels) {
    const model = {
      ...
      tasks: [...target.tasks],
      service_families:
        target.service_families === undefined ? undefined : [...target.service_families],
    } satisfies ProviderModel;
    models.set(model.model_id, {
      model,
      names: new Set([model.name, model.model_id, ...model.aliases]),
    });
  }
  ...
  applyPricing(
    ...
    input.source.id,
    bundle.index.body,
    pricingEvidence(bundle.documents),
    document("/gemini-enterprise-agent-platform/models/partner-models/claude/web-search")?.body,
    groundingReferences,
